...
```
```ensureAuthenticated``` e ```ensureAdmin``` são [**middlewares**](#middleware), que realiza a autenticação através do jwt e verifica se o usuário é administrador, respectivamente. 
